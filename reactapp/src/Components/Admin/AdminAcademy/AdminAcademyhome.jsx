...
function AdminAcademyhome() {
  ...
  useEffect(() => {
    axios.get('https://8080-abbaedcbbaecebadafdecbf.project.examly.io/admin/viewInstitutes').then((res) => {
      setData(res.data);
    }).catch((err) => {
      alert(err);
    })
  ...
  const handleDelete = (id) => {
    //Delete Theme Api
    axios.delete('https://8080-abbaedcbbaecebadafdecbf.project.examly.io/admin/deleteInstitute/' + id)
      .then(response => {
    ...
    if (result.isConfirmed) {
      try {
        const deleteRequests = selectedCards.map(id => {
          return axios.delete('https://8080-abbaedcbbaecebadafdecbf.project.examly.io/admin/deleteInstitute/' + id);
        });
