...
function AdminCourse2() {
  const navigate = useNavigate();
  const location = useLocation();
  const [courses, setCourses] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [vdata] = useState({
    courseId: location.state.instituteId,
  });

  useEffect(() => {
    const fetchCourses = () => {
      axios.get(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/admin/viewinstcoursebyId/${vdata.courseId}`)
        .then(response => {
  ...
  const handleSingleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/admin/deleteCourse/${id}`)
          .then((response) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const deleteRequests = selectedCards.map(id => {
          return axios.delete('https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/admin/deleteCourse/' + id);
