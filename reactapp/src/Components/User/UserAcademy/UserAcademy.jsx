...
const Viewacademy = () => {
  ...
  React.useEffect(() => {
    axios.get('https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/Getinstrat')
      .then(response => {
  ...
  const handleSearch = () => {
    ...
    const filteredCard = data.find((item) => {
      const title = item.instituteName || ""; 
  ...
  const handleCardClick = (instituteId) => {
    navigate(`/user/usercourse`,{state:{instituteId}});
  };
