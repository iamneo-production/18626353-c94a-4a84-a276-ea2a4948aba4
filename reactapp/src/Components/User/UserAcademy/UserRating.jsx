...
const Rating = () => {
  ...
  const getuserId = () => {
    ...
    axios
      .get(`https://8080-addebfabbbaecebadafdecbf.project.examly.io/user/${encodeURIComponent(email)}`)
      .then((response) => {
  ...
  const fetchReviews = async () => {
    try {
      const response = await axios.get(`https://8080-addebfabbbaecebadafdecbf.project.examly.io/GetRatingsForInstitute/${instituteId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post('https://8080-addebfabbbaecebadafdecbf.project.examly.io/RateInstitute', review);
      setRating(0);
      setComment('');
