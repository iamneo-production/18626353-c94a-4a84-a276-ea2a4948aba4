  ...
  useEffect(() => {
    ...
    if (isFirstRender) {
      ...
      if (!isFirstRender) {
        ...
        const fetchPreviousProgress = async () => {
          try {
            const response = await axios.get(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/user/viewStatus11?userid=${id}&courseid=${courseId}`);// get request to get the data from backend
    ...
    try {
      ...
      if (localStorage.getItem(`isFirstRender${courseid}`) === 'true') {
        ...
        await axios.post(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/user/viewstatusby?progresspercentage=${progress}&userid=${id}&courseid=${courseid}`);//for posting the data to backend
      } else {
        await axios.put(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/user/updatestatus/${progressid}?progressPercentage=${progress}`);//for updating the data at backend
      }
    } catch (error) {
      toast.error(error);
    }
