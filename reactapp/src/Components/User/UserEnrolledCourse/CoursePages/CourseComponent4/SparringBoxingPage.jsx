  ...
  useEffect(() => {
    ...
    if (isFirstRender) {
      ...
      if (!isFirstRender) {
        const fetchPreviousProgress = async () => {
          try {
            const response = await axios.get(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/user/viewStatus11?userid=${id}&courseid=${courseId}`);
    ...
    try {
      ...
      if (localStorage.getItem(`isFirstRender${courseid}`) === 'true') {
        ...
        await axios.post(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/user/viewstatusby?progresspercentage=${progress}&userid=${id}&courseid=${courseid}`);
      } else {
        await axios.put(`https://8080-caecdafbbaedcbbaecebadafdecbf.project.examly.io/user/updatestatus/${progressid}?progressPercentage=${progress}`);
      }
    } catch (error) {
      toast.error(error);
    }
      ...
      <div>
          ...
          <div style={{ flex: '1', overflowY: 'auto'}}>
              <div className='header4'>
                  <div className='titleMain4' style={{ marginLeft: '1%' }}><img width={120} height={80} src={logo} alt='Logo' /></div>
                  <div className='Heading4'>Sparring & Competitive Training</div>
              </div>
              <div className='bgimg4' style={{ width: '100', height: '100' }}>
              </div>
              <div className='decorate4'>
                  <div></div>
              </div>
              <div className='DescriptionStart4'>
              ...
              <div className='decoImg4' style={{ padding: '30px' }}>
              </div>
              <ToastContainer/>
          </div>
      </div></>
