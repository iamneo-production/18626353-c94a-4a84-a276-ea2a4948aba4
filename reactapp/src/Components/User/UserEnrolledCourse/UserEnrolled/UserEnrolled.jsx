...
function EnrolledCourse() {
  ...
  const getUserIdAndFetchAdmission = () => {
    ...
    axios.get(`https://8080-afbbeccfcdaebbaecebadafdecbf.project.examly.io/user/${encodeURIComponent(email)}`)
      .then((response) => {
  ...
  const getAdmissionData = (userId) => {
    axios.get(`https://8080-afbbeccfcdaebbaecebadafdecbf.project.examly.io/user/ViewAdmissionby?userId=${(userId)}`)
      .then((result) => {
  ...
  const handleDeleteCourse = (admissionId, result) => {
    if (result) { // Check if result is true or not
      axios
        .delete(`https://8080-afbbeccfcdaebbaecebadafdecbf.project.examly.io/user/deleteAdmission/${encodeURIComponent(admissionId)}`)
        .then((result) => {
          Swal.fire({
            icon: 'success',
